  ...
  async $notify(notify, ...rest) {
    for (const [callback, query] of this._subscriberMap) {
      if (query instanceof RegExp) {
        if (query.test(notify) && notify.constructor == String) {
          const result = await callback(notify, ...rest);
...
async function $SaveMetaFile(entryFile, outFile, fileManifest, writeMeta) {
  if (writeMeta === true) {
    ...
    await $fs2.writeFile(outFilePath.dir + "/" + outFilePath.name + ".meta", JSON.stringify(fileManifest));
  }
  const entryName = path2.parse(entryFile).base;
